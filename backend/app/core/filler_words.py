...
def analyse_fillers(transcript: str) -> Dict[str, Any]:
    ...
    top_fillers = sorted(
        [{"word": w, "count": c} for w, c in counts.items()], key=lambda x: x["count"], reverse=True
    )[:5]
...
def aggregate_filler_report(per_answer: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    avg_fluency = round(sum(r.get("fluency_score", 10) for r in per_answer) / len(per_answer), 2)
