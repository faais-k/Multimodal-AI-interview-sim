...
from backend.app.api.routes.cleanup          import router as cleanup_router   # P4-A
...
async def lifespan(app: FastAPI):
    ...
    # P4-A: Auto-cleanup sessions older than 48 hours on each startup.
    # Only deletes completed or abandoned sessions — never in-progress ones.
    try:
        from backend.app.api.routes.cleanup import cleanup_old_sessions
        # Direct Python call — bypasses header auth intentionally
        result = await cleanup_old_sessions(max_age_hours=48)
        print(f"🧹 Startup cleanup: deleted {result['deleted']} old sessions, "
...
app.include_router(cleanup_router,       prefix="/api")   # P4-A
...
def root():
    return {
        "message": "Multimodal AI Interview Simulator — Backend v0.4.0",
        "docs":    "/docs",
        "health":  "/api/health",
    }
