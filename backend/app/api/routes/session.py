...
async def start_interview(session_id: str):
    ...
    if not parsed_path.exists():
        raise HTTPException(status_code=404, detail="parsed_resume.json not found for session_id")
    try:
        parsed = json.loads(parsed_path.read_text(encoding="utf-8", errors="ignore"))
    except Exception:
        raise HTTPException(status_code=500, detail="failed to read parsed_resume.json")
