...
async def generate_analytics(session_id: str):
    ...
    if not fr_path.exists():
        raise HTTPException(
            status_code=404, detail="final_report.json not found. Call /aggregate first."
        )
    ...
    parsed_resume = (
        _read_json(session_dir / "parsed_resume.json")
        if (session_dir / "parsed_resume.json").exists()
        else {}
    )
    resume_skills = [
        str(s).strip().lower() for s in parsed_resume.get("skills", []) if str(s).strip()
    ]
    ...
    answer_quality = {
        ...
        "consistency": (
            "HIGH" if low_score_count <= 2 else "MEDIUM" if low_score_count <= 5 else "LOW"
        ),
    ...
    for sk in weak_areas:
        ...
        if expertise_level == "fresher":
            recommendations.append(
                f"Study {sk} fundamentals: review core concepts, complete a small project, and practise explaining it out loud."
            )
        elif expertise_level == "intermediate":
            recommendations.append(
                f"Deepen {sk} knowledge: work through a real-world problem end-to-end and document the trade-offs you considered."
            )
        else:
            recommendations.append(
                f"Strengthen {sk} depth: prepare a detailed example covering architecture decisions, performance optimisation, or failure handling."
            )
    ...
    if not_assessed_grouped:
        ...
        recommendations.append(
            f"Prepare answers for resume skills not yet assessed: {skills_str}. Use the STAR format for each."
        )
    if filler_summary.get("total_fillers", 0) > 0:
        common_words = ", ".join(
            [str(i.get("word", "")) for i in filler_summary.get("top_fillers", []) if i.get("word")]
        )
        recommendations.append(
    ...
    readiness_index = {
        ...
        "interview_quality": (
            "EXCELLENT" if final_score >= 8 else "GOOD" if final_score >= 6.5 else "WEAK"
        ),
    }
