...
async def _generate_dynamic_question(
    q_type: str, context: Dict, difficulty: str, company_research: Dict
) -> Dict:
    """
...
async def generate_dynamic_interview_logic(session_id: str):
    ...
    try:
        ...
        context = {
            ...
            "projects": [
                {
                    "name": _get_proj_field(p, "name", 50),
                    "description": _get_proj_field(p, "details", 200),
                }
                for p in projects[:3]
            ],
        ...
        for i, proj in enumerate(projects[:2]):
            proj_text = (
                _get_proj_field(proj, "details", 500) if isinstance(proj, dict) else str(proj)
            )
            proj_name = (
                _get_proj_field(proj, "name", 100) if isinstance(proj, dict) else f"Project {i+1}"
            )

            proj_context = {
                **context,
                "current_project": proj_text[:300],
                "project_tech": re.findall(
                    r"\b(Python|JavaScript|React|Node|MongoDB|SQL|AWS|Docker|Kubernetes)\b",
                    proj_text,
                    re.I,
                ),
            }
        ...
        for i, skill in enumerate(tech_skills):
            tech_context = {**context, "target_skill": skill}
        ...
        critical_context = {**context, "target_skill": critical_skill}
