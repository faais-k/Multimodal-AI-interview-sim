...
def _project_info(raw_project: dict | str, fallback_idx: int) -> Tuple[str, List[str]]:
    ...
    # Check if contains mostly generic words
        ...
        sum(1 for w in candidate_words if w.lower() in _GENERIC_NAMES) >= len(candidate_words) * 0.5
    ):
        is_generic = True
    ...
    if is_generic:
        ...
        potential_names = re.findall(
            r"\b([A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*){1,4})\b", clean_text[:200]
        )
...
_PROJ_DATE_RE = re.compile(
    r"\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\s+20", re.IGNORECASE
)
_PROJ_VERB_RE = re.compile(
    r"\b(?:technologies|built|developed|implemented|created|designed|deployed)\b", re.IGNORECASE
)
...
def _filter_valid_projects(projects: List[dict | str]) -> List[dict | str]:
    ...
    for p in projects:
        ...
        if len(t) < 15:
            continue
        if any(kw in t.lower() for kw in _PROJ_EDU_KW):
            continue
        if re.match(
            r"^[•\-\*]\s*(?:b\.sc|b\.tech|m\.sc|m\.tech|b\.e|m\.e|mba|bachelor|master)", t.lower()
        ):
            continue
        if _PROJ_DATE_RE.search(t) and not _PROJ_VERB_RE.search(t):
            continue
        valid.append(p)
    return valid
...
def _pick_skills(resume_skills, role_text, jd_text, count):
    ...
    scored = sorted(
        rs,
        key=lambda s: (
            len(set(_extract_keywords(s.lower(), 4)) & role_tokens) + random.uniform(0, 0.3)
        ),
        reverse=True,
    )
...
def _pregenerate_followups(questions: list, job_role: str, expertise_level: str) -> dict:
    try:
        ...
        for q in questions:
            ...
            style = (
                "Ask for a specific example from their coursework or personal projects."
                if expertise_level == "fresher"
                else (
                    "Ask about a concrete challenge they faced and how they resolved it."
...
async def create_interview_plan(session_id: str):
    ...
    if not parsed_p.exists():
        raise HTTPException(
            status_code=404, detail="Parsed resume not found. Run /parse/resume first."
        )
    parsed = _read_json(parsed_p)

    jd = (
        _read_json(sdir / "job_description.json")
        if (sdir / "job_description.json").exists()
        else {}
    )
    prof = (
        _read_json(sdir / "candidate_profile.json")
        if (sdir / "candidate_profile.json").exists()
        else {}
    )
    ...
    target_skills = _pick_skills(
        resume_skills, job_role, job_description or job_role, EXPERTISE_TECH_COUNT[expertise_level]
    )
    ...
    questions.append(
        {
            "id": "intro_1",
            "type": "self_intro",
            "question": _intro(candidate_name, job_role),
        ...
        questions.append(
            {
                "id": "project_1",
                "type": "project",
                "question": _project_q(p1, s1, job_role, expertise_level),
                "skill_target": p1,
                "source": "rule",
            }
        )
            ...
            questions.append(
                {
                    "id": str(uuid.uuid4()),
                    "type": "project",
                    "question": _project_q(p2, s2, job_role, expertise_level),
                    "skill_target": p2,
                    "source": "rule",
                }
            )
        ...
        questions.append(
            {
                ...
                "question": random.choice(
                    [
                        "Walk me through the most technically challenging thing you've built and your exact contribution.",
                        "Tell me about a significant technical challenge you tackled — from coursework, open source, or self-learning.",
                    ]
                ),
        ...
        questions.append(
            {
                "id": str(uuid.uuid4()),
                "type": "technical",
                "question": _technical_q(skill, job_role, expertise_level),
                "skill_target": skill,
                "source": "rule",
            }
        )
    ...
    questions.append(
        {
            "id": "behavioral_1",
            "type": "hr",
            "question": _behavioral_q(expertise_level),
    ...
    scenario = (
        target_skills[0]
        if target_skills
        else (resume_skills[0] if resume_skills else "your tech stack")
    )
    questions.append(
        {
            "id": "critical_1",
            "type": "critical",
            "question": _critical_q(scenario, job_role, expertise_level),
            "skill_target": scenario,
            "source": "rule",
        }
    )
    ...
    try:
        followup_cache = await asyncio.wait_for(
            asyncio.to_thread(_pregenerate_followups, questions, job_role, expertise_level),
            timeout=90.0,
        )
